...
local function IS_make_lakes(x, y, tile, map, options)
  ...
  local lakes = simple_variable_persistence_multioctave_noise{
    ...
    octave0_input_scale = (1/2) * island_frequency,  -- Higher number means more islands
...
data:extend{
  {
    ...
    expression = noise.define_noise_function( function(x,y,tile,map)
      ...
      options =
      {
        bias = water_level,  -- Higher number (closer to 0) means water level is lower so islands are bigger
        terrain_octaves = 10
      }
